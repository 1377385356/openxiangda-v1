...
- ✅ 未登记环境的旧工作区，正式 promotion 先聚合 mainline bundle 并 commit/push，再运行 `release publish --change <id> --profile <name>`。Legacy Page step 必须遵守 CLI 注入的 `OPENXIANGDA_PAGE_STAGE_ONLY=1` 并通过当前包内 CLI 登记 staged PageRelease；缺少证据时 Root finalize 会失败关闭。平台核验精确非删除目标来自多次历史 lineage 后，可成对增加 `--adopt-online-baseline --adoption-reason "..."`；参数只进入精确 `resource publish --only/--code` 阶段。需要完整替换 Function/Automation manifest 时可成对增加 `--replace-manifest --reason "..."`，只进入唯一的精确 Backend stage。上述参数不进入 ensure、Page 或 App finalize，无兼容精确范围时在获取租约前失败关闭。
